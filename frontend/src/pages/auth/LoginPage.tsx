import { useState, useEffect } from 'react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { authApi } from '@/api/auth.api';
import useAuthStore from '@/store/authStore';
import { API_URL } from '@/lib/config';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { PasswordInput } from '@/components/ui/password-input';
import { Brand, Tape } from '@/components/decor';
import { getApiErrorMessage } from '@/lib/errors';

export default function LoginPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const setAuth = useAuthStore((s) => s.setAuth);
  const user = useAuthStore((s) => s.user);
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [waking, setWaking] = useState<'github' | 'google' | null>(null);

  // The backend redirects here with ?error=oauth_unregistered when someone signs
  // in with Google/GitHub whose email the teacher has not added.
  useEffect(() => {
    if (searchParams.get('error') === 'oauth_unregistered') {
      setError(
        'החשבון הזה אינו רשום במערכת. פני למורה כדי שתוסיף אותך. ' +
        'לניסיון עם חשבון Google אחר — לחצי שוב, ייפתח מסך בחירת חשבון. ' +
        'לניסיון עם חשבון GitHub אחר, ייתכן שתצטרכי להתנתק מהחשבון הזה גם באתר GitHub עצמו.'
      );
    }
  }, [searchParams]);

  // A restored session landing here (e.g. the old bootstrap race, or a manual
  // /login visit) should go straight in rather than ask for a password again.
  useEffect(() => {
    if (!user) return;
    if (user.mustChangePassword) navigate('/change-password', { replace: true });
    else navigate(user.role === 'ADMIN' ? '/teacher' : '/student', { replace: true });
  }, [user, navigate]);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError('');
    setLoading(true);
    try {
      const res = await authApi.login(email, password);
      const { user, accessToken } = res.data.data;
      setAuth(user, accessToken);
      if (user.mustChangePassword) {
        navigate('/change-password');
      } else {
        navigate(user.role === 'ADMIN' ? '/teacher' : '/student');
      }
    } catch (err: any) {
      setError(getApiErrorMessage(err, 'שגיאה בכניסה למערכת. אנא בדקו את הפרטים ונסו שוב.'));
    } finally {
      setLoading(false);
    }
  }

  async function handleOAuth(provider: 'github' | 'google') {
    // A spun-down free-tier backend would otherwise intercept the OAuth
    // redirect with Render's own "waking up" placeholder page — so wake it
    // here first (health check has no auth/DB dependency) and only then hand
    // off to the backend's OAuth route, once it can actually respond.
    setWaking(provider);
    await wakeBackend();
    // Full-page navigation straight to the backend (not the SPA host) so the OAuth
    // redirect chain and the refresh cookie all live on the backend's origin.
    window.location.href = `${API_URL}/auth/${provider}`;
  }

  async function wakeBackend() {
    const deadline = Date.now() + 20_000;
    while (Date.now() < deadline) {
      try {
        const res = await fetch(`${API_URL}/health`);
        if (res.ok) return;
      } catch {
        // still asleep / spinning up — retry until the deadline
      }
      await new Promise((r) => setTimeout(r, 1500));
    }
  }

  return (
    <div className="flex min-h-screen items-center justify-center bg-graph p-4" dir="rtl">
      <div className="relative w-full max-w-sm">
        <div className="mb-6">
          <Brand variant="hero" teacher />
        </div>

        <div className="relative rounded-xl border border-rule bg-sheet p-6 shadow-lift">
          <Tape color="clay" rotate={-4} className="-top-2.5 right-10 w-20" />
          <Tape color="indigo" rotate={4} className="-top-2.5 left-10 w-16" />
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <Input
              label="כתובת אימייל"
              type="email"
              placeholder="you@example.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              required
              autoFocus
            />
            <div className="flex flex-col gap-1">
              <PasswordInput
                label="סיסמא"
                placeholder="••••••••"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                required
              />
              <Link to="/forgot-password" className="self-start text-xs text-clay hover:underline">
                שכחתי סיסמא
              </Link>
            </div>

            {error && (
              <p className="border border-coral bg-coral/10 px-3 py-2 font-sans text-sm text-coral">
                {error}
              </p>
            )}

            <Button type="submit" variant="clay" loading={loading} className="mt-1 w-full" size="lg">
              כניסה למערכת
            </Button>
          </form>

          <div className="my-5 flex items-center gap-3">
            <div className="h-0.5 flex-1 border-t border-dashed border-rule/30" />
            <span className="font-sans text-[11px] uppercase text-ink/50">או המשיכי עם</span>
            <div className="h-0.5 flex-1 border-t border-dashed border-rule/30" />
          </div>

          <div className="flex flex-col gap-2">
            <button
              type="button"
              disabled={waking !== null}
              onClick={() => handleOAuth('google')}
              className="flex w-full items-center justify-center gap-2 border border-rule bg-sheet py-2 text-sm font-bold text-ink shadow-soft lift disabled:opacity-60"
            >
              {waking === 'google' ? (
                <span className="text-xs text-ink-soft">מעירה את השרת… זה עלול לקחת עד כ-20 שניות</span>
              ) : (
                <>
                  <svg width="18" height="18" viewBox="0 0 48 48">
                    <path fill="#EA4335" d="M24 9.5c3.5 0 6.6 1.2 9.1 3.2l6.8-6.8C35.8 2.5 30.3 0 24 0 14.8 0 6.9 5.4 3 13.3l7.9 6.1C12.9 13.2 17.9 9.5 24 9.5z"/>
                    <path fill="#4285F4" d="M46.5 24.5c0-1.6-.1-3.1-.4-4.5H24v8.5h12.7c-.6 3-2.3 5.5-4.8 7.2l7.5 5.8c4.4-4 7.1-10 7.1-17z"/>
                    <path fill="#FBBC05" d="M10.9 28.6A14.5 14.5 0 019.5 24c0-1.6.3-3.2.8-4.6L2.4 13.3A23.9 23.9 0 000 24c0 3.8.9 7.4 2.5 10.6l8.4-6z"/>
                    <path fill="#34A853" d="M24 48c6.5 0 11.9-2.1 15.9-5.8l-7.5-5.8c-2.2 1.5-5 2.3-8.4 2.3-6.1 0-11.2-4.1-13-9.7l-8.4 6C6.9 42.6 14.8 48 24 48z"/>
                  </svg>
                  המשיכי עם Google
                </>
              )}
            </button>

            <button
              type="button"
              disabled={waking !== null}
              onClick={() => handleOAuth('github')}
              className="flex w-full items-center justify-center gap-2 border border-rule bg-sheet py-2 text-sm font-bold text-ink shadow-soft lift disabled:opacity-60"
            >
              {waking === 'github' ? (
                <span className="text-xs text-ink-soft">מעירה את השרת… זה עלול לקחת עד כ-20 שניות</span>
              ) : (
                <>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.44 9.8 8.2 11.4.6.1.82-.26.82-.57v-2c-3.34.73-4.04-1.6-4.04-1.6-.54-1.4-1.33-1.77-1.33-1.77-1.08-.74.08-.72.08-.72 1.2.08 1.83 1.23 1.83 1.23 1.06 1.82 2.78 1.3 3.46.99.1-.77.42-1.3.76-1.6-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.14-.3-.54-1.52.1-3.18 0 0 1-.32 3.3 1.23a11.5 11.5 0 016 0c2.28-1.55 3.29-1.23 3.29-1.23.65 1.66.24 2.88.12 3.18.77.84 1.23 1.91 1.23 3.22 0 4.61-2.8 5.63-5.48 5.92.43.37.81 1.1.81 2.22v3.29c0 .32.22.68.83.57C20.57 21.8 24 17.3 24 12c0-6.63-5.37-12-12-12z"/>
                  </svg>
                  המשיכי עם GitHub
                </>
              )}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
