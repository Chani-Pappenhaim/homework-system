...
**1. "התצוגה המקדימה רק בקטן בצד."** מילולית מה שהיה: `FilePreviewInline` בעמודה דביקה
`lg:w-80` (320px) לצד הרשת. הוחלף ב-`Dialog` עם `size="full"` חדש ב-`dialog.tsx`
(`w-[calc(100vw-2rem)] h-[calc(100vh-2rem)]`, עם `max-h-none` שמבטל את התקרה של 90vh).
כל היכולות של main נשמרו: Office דרך gview, `txt`/`md` ב-fetch, קובץ חובה/נצפה, שינוי שם.

**2. "ההורדה בשם מוזר ובפורמט לא נתמך."** שני שורשים:
- **שם התצוגה נשמר ב-DB בלי סיומת** — `FileUpload` מוריד אותה כברירת מחדל, ולכן ב-DB יושב
  `"חוזה"` וה-`Content-Disposition` נבנה ממנו. הסיומת מושבת עכשיו מה-URL (`ensureExtension`).
  **ה-DB לא נגע** — שם התצוגה נשאר יפה, רק ההורדה מקבלת `חוזה.pdf`.
- **`Content-Type` נלקח מ-Cloudinary**, שמחזיר `application/octet-stream` לנכס `raw` →
  "פורמט לא נתמך". עכשיו `resolveContentType` מעדיף את מה ש-Cloudinary אומר, **אלא אם** זה
  octet-stream גנרי — ואז נגזר מהסיומת (`utils/mime.ts` חדש).

**3. נטפרי.** אין דרך לגיטימית "לא לשלוח לבדיקה" — הסינון הוא של הרשת. מה שכן: **בקשת
פתיחת דומיין**, נוסח מוכן ב-`בקשה-לנטפרי.md` בשורש עם שני הדומיינים
(`homework-system-mocha.vercel.app` + `homework-system-3haq.onrender.com`).

**🔴 באג שלא היה ידוע — `<iframe>` של PDF חסום בפרודקשן.** helmet מחזיר
`X-Frame-Options: SAMEORIGIN` + CSP `frame-ancestors 'self'` על **כל** תגובה (אומת מול
הפרודקשן החי), וה-SPA יושב בדומיין אחר — כלומר תצוגת PDF לא יכלה לעבוד שם מעולם.
תגובת קובץ **מסירה** את שתי הכותרות; svg/html/xml מקבלים במקומן CSP עם `sandbox` (קובץ
כזה רץ על הדומיין של ה-API, שם יושב ה-refresh cookie).

**🔴 נטפרי חוסמת בקשות `Range`** — מחזירה **418 עם דף חסימה** (אומת בתוך הקונטיינר).
נוסף fallback לקובץ המלא. בפרודקשן אין נטפרי בדרך וה-206 עובד כרגיל.
נוסף גם טיפול בשגיאת stream באמצע (לא היה) — כותרות כבר על הקו, אז רק לוג + ניתוק.

**⚠️ ממצא שלא טופל (לא בסקופ):** `files.controller` מביא **כל URL** שיושב בשורה, ו-URL
בזרימת ההעלאה הישירה מגיע מהלקוח (`uploadedFile.url`) → וקטור SSRF. שקלתי allowlist
ל-`res.cloudinary.com` ולא הוספתי, כדי לא להפיל קובץ קיים בפרודקשן שכתובתו לא צפויה.
**שווה החלטה משותפת.**

**אימות:** שני ה-tsc נקיים (נדרש `npx prisma generate` — ה-client המקומי היה מיושן מול
הסכמה של main) · backend 320 עוברות, 1 נכשלת **מראש ב-main** (`controllers.test.ts`,
`files/:fileId/required`) — אומת עם ובלי השינוי · frontend **264/264** · 6 בדיקות backend
חדשות ל-route + קובץ `mime.test.ts` (13) + `FileGallery.test.tsx` (7) חדש ·
**אומת חי ב-Docker:** שורה ב-`LessonFile` עם שם בלי סיומת הוגשה עם
`Content-Disposition: ...filename*=UTF-8''הוראות.md` ובלי `X-Frame-Options`/CSP.

**⚠️ למה זה נבנה פעמיים:** העבודה התחילה על ענף מ-`main` מקומי מיושן, ובמקביל **אותו פיצ'ר
כבר נבנה ב-`origin/main`** (טוקן קובץ + בדיקת הרשאה + required/viewed). התגלה רק בבדיקה
לפני push. הענף הראשון `feat/file-preview-and-download` **נזנח בכוונה** — אל תמזגו אותו.
נרשם חוק זיכרון: לפני כל משימה `git fetch` ולבדוק אם `origin/main` התקדם.

**נשאר:** בדיקה ויזואלית בדפדפן (Playwright לא מותקן); `.env` המקומי מחזיק מפתח Cloudinary
פגום (`Invalid api_key`) ולכן העלאה מקומית נכשלת — האימות נעשה מול URL חיצוני;
שורת בדיקה `11111111-1111-1111-1111-111111111111` ב-`LessonFile` המקומי (מצביעה ל-readme
ב-GitHub) — למחוק כשלא צריך; ה-thumbnail של כל אריח טוען `<iframe>` מלא (PDF/gview) —
כבד, שווה בדיקה בעתיד.
...
**✅ [LessonDetailPage.tsx](../frontend/src/pages/teacher/LessonDetailPage.tsx) — הטאב-טוגל עבר מ-2 מצבים (`content`/`access`) ל-3 (`content`/`assignments`/`access`).**
- טאב "תוכן שיעור" (`content`): נשאר רק תוכן השיעור + קישורי GitHub + קבצים מצורפים/העלאה.
- טאב חדש "מטלות" (`assignments`, אייקון `ClipboardList`): ה-Card של רשימת המטלות + טבלת הגשות (`AssignmentSubmissionsTable`) שהיה קודם בתוך טאב ה-content עבר לכאן במלואו.
- טאב "הרשאות וחידון AI" (היה "הרשאות גישה", שונה שם): מכיל עכשיו גם את `LessonAccessPanel` וגם את `QuizResultsCard` (יצירת/ניהול חידון AI לשיעור) שהיה קודם מתחת למטלות בטאב content.
- deep-link מדוח הציונים (`?assignmentId=&submissionId=`) — ה-`useEffect` שמאתר את המטלה לפי `targetAssignmentId` עכשיו גם קורא ל-`setTab('assignments')` כדי שהתלמידה/מורה תנחת ישר על הטאב הנכון, לא רק על המטלה הנכונה בתוך טאב שאולי לא פתוח.

**✅ [file-gallery.tsx](../frontend/src/components/ui/file-gallery.tsx) — תצוגה מקדימה של קובץ עברה מ"מתחת לרשת התמונות" ל"בצד" (side panel), לפי בקשה מפורשת.**
- ה-container החיצוני: `flex flex-col` (נשאר עמודה כשאין קובץ נבחר), אבל כש-`selected` קיים נהיה `lg:flex-row-reverse lg:items-start` (RTL: הפאנל מוצג בצד ימין ויזואלית, הרשת תופסת את שאר הרוחב).
- רשת הקבצים מקבלת `lg:flex-1` כש-יש קובץ נבחר.
- `FilePreviewInline` קיבל prop חדש `className` (מוזרם מבחוץ עם `lg:sticky lg:top-4 lg:w-80 lg:shrink-0`) כדי שיתנהג כפאנל צר קבוע-רוחב שדבוק (`sticky`) בגלילה, במקום פאנל ברוחב מלא. גובה התצוגה הפנימית קוצר מ-`min-h-96` ל-`min-h-72` כי הפאנל צר יותר עכשיו.
- **לא טופל בסבב הזה** (נצפה בצילום המסך אך לא אובחן/תוקן): שני תגי-קובץ הציגו overlay "NETFREE ▾ Language" במקום תוכן אמיתי — זה כנראה חסימת/יירוט של הרשת (NetFree SSL inspection, ראה CLAUDE.md) על בקשת ה-preview (למשל ה-iframe ל-`docs.google.com/gview` למסמכי Office, או בקשת ה-`fetch` לתצוגת טקסט/PDF), לא קשור לקוד. צריך לבדוק מול רשת אחרת/מחוץ ל-NetFree כדי לאשר שזו אכן הסיבה.
- אומת עם `npm run build` בפרונט — נקי (`✓ built in 14.19s`).

**⚠️ טרם נבדק חי בדפדפן/Docker** — כל התיקונים בסבב הזה (וגם התיקון הקודם של הורדה/נגינת קבצים מהסבב הקודם) אומתו רק ב-build מקומי, לא הורצו בפועל מול Docker/production. המשתמש התבקש להריץ `docker compose -p homework-app up -d` בעצמו (לפי כלל CLAUDE.md - לא מריצים Docker בשבילו).

## 2026-09-15 (המשך 5) — חלון הודעה צף לתלמידה, תיקון אמיתי להורדה/נגינה של קבצים, הוסר טאב "הרשאות גישה" הריק

בעקבות דיווח עם צילום מסך + "תתקן את הכל" (3 נושאים):

**✅ StudentMessagesPage — הודעה למורה עכשיו נשלחת דרך חלון צף (Dialog), כמו אצל המורה.** הוסר הטופס המפוצל (Card עם textarea בראש העמוד ליד רשימת ההודעות). נוסף כפתור "הודעה חדשה" ב-`PageHeader actions` שפותח `composeOpen` Dialog עם Textarea+שליחה/ביטול — בדיוק תבנית `teacher/MessagesPage.tsx` (מותאם: אין בורר נמענת, התלמידה תמיד פונה ל"המורה" בלבד). נוסף גם מצב טעינה עם `MessageSkeleton` ו-`EmptyState` לרשימה ריקה (חסרו קודם).

**✅ נמצא ותוקן שורש בעיית "קובץ שיר לא נפתח" + "הורדה לא נתמכת בבדיקה האוטומטית".** לא היה קשור לסיווג סוג הקובץ (זה כבר תוקן קודם) — הבעיה הייתה ב-[files.controller.ts](../backend/src/controllers/files.controller.ts): ה-endpoint עשה `res.redirect()` ישירות ל-Cloudinary CDN (origin שלישי, לא אותו origin כמו ה-frontend/ה-API). שני תקלות נובעות מזה:
1. **הורדה:** `<a href download>` על יעד cross-origin — הדפדפן **מתעלם** מתכונת ה-`download` (מתנהג כניווט/פתיחה רגילה במקום שמירה בפועל). זה בדיוק מה שבדיקה אוטומטית שמצפה לקובץ שיורד בפועל הייתה תופסת כ"לא נתמך".
2. **נגינת אודיו/וידאו:** redirect ל-CDN חיצוני עלול להיתקל במדיניות CORS/referrer מחמירה יותר בדפדפן/תוכנת אבטחה מקומית מול `<audio>`/`<video>` שמפנה ל-origin שלישי.

**התיקון:** ה-controller כבר לא עושה redirect — הוא שולף את הקובץ בעצמו מ-Cloudinary (`fetch(toDeliveryUrl(...))`, Node global fetch, אותה טכניקה שכבר בשימוש ב-`code-extraction.ts`) וזורם (`Readable.fromWeb(...).pipe(res)`) אותו ללקוח **מאותו origin כמו שאר ה-API** — עם `Content-Type`/`Content-Length` מהתגובה של Cloudinary, ו-`Content-Disposition` (`inline` כברירת מחדל, `attachment` כש-`?dl=1` בבקשה — כולל `filename*=UTF-8''` לשמות עבריים). ב-frontend, [file-gallery.tsx](../frontend/src/components/ui/file-gallery.tsx) — שני קישורי ה"הורדה" (התצוגה הרגילה + הפולבק ל-archive/other) עכשיו מוסיפים `&dl=1`/`?dl=1` ל-URL; קישורי התצוגה (`<img>`/`<audio>`/`<video>`/`<iframe>` src) נשארו ללא שינוי (מקבלים `inline`).

⚠️ **לא אומת בפועל בדפדפן חי מול production** (אין גישה לקובץ שיר אמיתי/DB production מהסביבה של הסוכן) — `tsc --noEmit`/`npm run build` נקיים בשני הצדדים, אבל **המשתמשת צריכה לוודא בפועל אחרי הפריסה** שקובץ השיר מתנגן וש-"הורדה" שומרת קובץ אמיתי (לא רק פותחת בטאב חדש).

**✅ הוסר טאב "הרשאות גישה" הריק ב-`CourseDetailPage.tsx`** (נשאר ריק מהתקלה הקודמת שבה `CourseAccessPanel` שהוזכר בייבוא מעולם לא נבנה, וגרם לכשל build ב-Render). נשאלה המשתמשת אם לבנות רכיב אמיתי (כולל בוחן AI ברמת קורס שהוזכר בעבר) או להסיר — היא ענתה "למה צריך את זה? אולי להסיר וזהו" — הוסר לגמרי ה-tab-toggle (content/access), העמוד חזר לתצוגה יחידה כמו שהיה לפני שנוסף. **פיצ'ר בוחן-AI-לכל-הקורס (שהוזכר בשיחה קודמת) לא מומש ולא בסקופ יותר**, אלא אם המשתמשת תבקש מפורשות בעתיד.

**Commit `70cdbc7`, נדחף ל-main.** `npm run build` נקי בשני הצדדים (frontend+backend).

**נשאר לעשות:**
- אימות בדפדפן חי (production) של תיקון האודיו/הורדה — לא בוצע מסיבת גישה.
- לוודא שהמשתמשת רואה את השינויים העיצוביים אחרי שה-deploy האחרון (Vercel/Render) מסתיים — היא שאלה "האם זה קשור לכך שעוד לא גמר להיבנות" ולא קיבלה תשובה סופית, רק אישור ש-build מקומי עובד.
...
**⚠️ השלכה על פיצ'ר שינוי-השם:** מעכשיו והלאה זה כבר לא רלוונטי (הסיווג לא תלוי בשם), אבל כדאי לזכור שהבאג היה קיים גם לפני התכונה — כל קובץ שהועלה עם שם ידידותי בלי סיומת נפגע.

**✅ Docker אושר כעובד** אצל המשתמשת (אחרי שפתחה את Docker Desktop בעצמה, לא רק פקודות).

**✅ בדיקת DB לקבוצה יג הורצה** (`backend/scratch-check-group.js`, על ידי המשתמשת דרך הפקודה שסיפקתי — `DATABASE_URL` מתיקיית backend): קבוצה נוצרה 11.7.2026 20:48. שתי תלמידות: רבקה לוי (rivkalevi@gmail.com, הצטרפה 20:48) ויהודית כהן (y5656@gmail.com, הצטרפה 20:49). ניתן למחוק את `backend/scratch-check-group.js` אם לא צריך אותו יותר.

**✅ סטטוס אבטחה דווח למשתמשת:** סגור — הגנת Multer מפני DoS, zip bomb, JWT_SECRET (commit `532147a`). עדיין פתוח (נדחה במפורש בעבר לפי בקשתה) — פער `expires_at`/redirect endpoint לקישורי הורדה חתומים. מחכה להחלטתה אם לטפל עכשיו.

**עדיין לא התחיל:** תוכנית עיצוב מחדש לדשבורד (מחכה שאציג תוכנית ותאשר לפני קוד); בירור איזון עמוד השיעור (LessonDetailPage) — ביקשתי צילום מסך/אישור, טרם התקבל.
