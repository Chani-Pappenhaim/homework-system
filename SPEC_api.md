...
## Files `/api/files` — הגשת קבצים מאוחסנים

| Method | Path | Auth | תיאור |
|---|---|---|---|
| GET | `/download/:fileId?token=…` | טוקן קובץ | מזרים את הקובץ דרך הדומיין שלנו. `?dl=1` → `attachment`, אחרת `inline` |

**הרשאה:** הקישור נושא **טוקן קצר-מועד משלו** (`signFileToken`, 15 דק'), כי `<a>`/`<img>`/
`<iframe>` לא יכולים לשאת כותרת `Authorization`. ההרשאה עצמה (קבוצה / `LessonAccess`)
**נבדקת מחדש מול ה-DB בכל הורדה**, כך שתלמידה שההרשאה שלה בוטלה אחרי הנפקת הקישור לא
תוכל להשתמש בו. `toFileDTO` הוא שמייצר את ה-URL הזה, עם `extension` לצדו.

**למה בכלל proxy ולא קישור ישיר ל-Cloudinary:**
- `<a download="...">` **מתעלמים ממנו בין דומיינים** — הקובץ נשמר בשם ה-public_id.
- Cloudinary חוסם הגשה לא-חתומה של נכסי `raw` ו-PDF.
- סינון תוכן ברשת מאשר **דומיין**, ו-CDN משותף אף אחד לא יאשר.

**שם וסוג:**
- שם התצוגה נשמר ב-DB **בלי סיומת** (טופס ההעלאה מוריד אותה), ולכן הסיומת מושבת מה-URL
  לפני בניית `Content-Disposition` — בלי זה הקובץ יורד כ-`contract` ולא נפתח בכלום.
- `Content-Type` נלקח מ-Cloudinary, **אלא אם** הוא `application/octet-stream` — אז הוא נגזר
  מהסיומת (`resolveContentType`).
- `Content-Disposition` נושא שם בעברית ב-`filename*=UTF-8''` עם fallback ASCII **עם סיומת**.

**הטמעה בדפדפן:** ה-SPA יושב בדומיין אחר (Vercel) מה-API (Render), ולכן תגובת קובץ
**מסירה** את `X-Frame-Options` ואת ה-CSP של helmet — בלעדי זה `<iframe>` של PDF נחסם.
סוגים שהדפדפן מריץ כמסמך (svg/html/xml) מקבלים במקום זה CSP עם `sandbox`.

**Range:** מועבר הלאה (206) כדי ש-`<audio>`/`<video>` ינגנו ויאפשרו דילוג. אם הבקשה עם
`Range` נדחית ע"י גורם ביניים — **נסיגה לקובץ המלא** (רשת נטפרי מחזירה 418 עם דף חסימה).
