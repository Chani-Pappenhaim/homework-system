...
- **מגבלה:** פעם אחת למטלה. מורה יכולה לאשר עוד אחת חד-פעמית (`aiExtraAllowed=true`) — כפתור "אפשרי בדיקת AI נוספת" במודל הציון של המורה. כשתלמידה מגיעה למגבלה (`aiReviewCount >= maxReviews`, השרת מחזיר `'AI review limit reached'`) — ה-UI שלה מציע לשלוח הודעה למורה עם בקשה, כמו בקשת הגשה מאוחרת
- **aiCodeReview** — נראה לתלמידה מיד (לא דורש אישור); **גם למורה** במודל הציון (כפתור "הצגי הערות קוד", מוסתר כברירת מחדל)
...
### הודעות (TeacherMessage) — מחיקה + סטטוס "נקרא"
- `isRead` — האם המורה קראה את הודעת התלמידה (בסיס לפעמון של המורה)
- `replySeen` — האם התלמידה פתחה את דיאלוג ההודעה אחרי שהמורה הגיבה (בסיס לפעמון של התלמידה). מתאפס ל-`false` בכל פעם שהמורה שולחת/מעדכנת תגובה
- מחיקה: מורה יכולה למחוק שיחה שלמה או רק את התגובה שלה (ההודעה המקורית חוזרת ל"ממתין"); תלמידה יכולה למחוק רק הודעה שהיא עצמה שלחה
...
המורה                                    התלמידה
─────────────────────────────            ──────────────
[צרי בוחן בעזרת AI]  ← ADMIN בלבד
      ↓  published=false (טיוטה)          "עדיין לא זמין"
[עריכת שאלות ותשובות]                     (הטיוטה בלתי-נראית)
      ↓
[פרסמי לתלמידות]  ──────────────────────►  [10 שאלות]
      ↓
[החזירי לטיוטה]  ───────────────────────►  "עדיין לא זמין"
```

- **קריאת GET לא מייצרת בוחן.** רק `POST /generate` (ADMIN). תלמידה לא מפעילה ולא ממתינה לקריאת AI.
- **טיוטה בלתי-נראית לחלוטין.** לתלמידה, בוחן שלא פורסם ובוחן שלא קיים מחזירים תשובה זהה.
- **`POST /attempt` נכשל ב-409 על בוחן שלא פורסם** — לא רק ה-GET מסתיר אותו.
- **עריכה מוחקת את כל הניסיונות** (בטרנזקציה אחת): ציון שמור מתייחס לשאלות שכבר לא קיימות.
- **אין `regenerate`.** `POST /generate` מחזיר 409 כשכבר קיים בוחן — יצירה מחדש הייתה מוחקת בשקט
  שאלות שהמורה ערכה. שינויים נעשים דרך העורך.
- ה-worker כותב תמיד `published: false`.

**ולידציה** (`validateQuestions`) — רצה גם בשמירה וגם בפרסום: לפחות שאלה אחת, טקסט לא ריק,
לפחות שתי אפשרויות עם טקסט, ו-`correctIndex` שלם בטווח. **ביטול** פרסום מותר תמיד — זו הדרך
לצאת מבוחן תקול.

**סטטוסים של `GET /lessons/:id/quiz`:**
| status | מי רואה | מתי |
|---|---|---|
| `ready` | שניהם | יש בוחן (למורה — גם טיוטה, כולל `correctIndex`) |
| `none` | ADMIN | אין בוחן, יש `contentMd` → אפשר ליצור |
| `generating` | ADMIN | job בתור/רץ |
| `failed` | ADMIN | היצירה נכשלה, כולל הסיבה הטכנית |
| `unavailable` | שניהם | למורה: אין `contentMd`. לתלמידה: אין בוחן **או** שהוא טיוטה |
